...
int main( int argc, char * argv[])
{
  ...
  filebuf in_file_buff;
  in_file_buff.open( input_filename, ios_base::in );
  istream in_file(  &in_file_buff );
  ...
  int chars_read;
  while( chars_read = ( in_file.get( charblock, 9, EOF ) ).gcount() )
  {
    string block( charblock );
  
    if( encrypt_mode && chars_read != 8 )
    {
      for( int i = chars_read; i <= 8; i++ )
        block[ i ] = 0;
    }
    ...
    else //Decrypt
    {
      ...
      for( int i = 7; i >= 0; i-- )
      {
        ...
        if( char( out_bits[ i ].to_ulong() ) != 0 ) //If char is not null
          out_stream << char( out_bits[ i ].to_ulong() );
      }   
    }
  }
